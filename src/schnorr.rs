...
use crate::utils::field_elem_from_try_and_incr;
...
mod tests {
    ...
    fn schnorr_single() {
        ...
        fn check_equality_g1() {
            // ($protocol_name:ident, $proof_name: ident, $group_affine:ident, $group_projective:ident) => {
                impl_proof_of_knowledge_of_discrete_log!(Protocol3, Proof3);
                let mut rng = StdRng::seed_from_u64(0u64);
                let gbase = <Bls12_381 as Pairing>::G1::rand(&mut rng).into_affine();
                let z1 = Fr::rand(&mut rng);
                let z2 = Fr::rand(&mut rng);
                let base1 = gbase.mul_bigint(z1.into_bigint()).into_affine();
                let base2 = gbase.mul_bigint(z2.into_bigint()).into_affine();;
                let witness = Fr::rand(&mut rng);
                let y1 = base1.mul_bigint(witness.into_bigint()).into_affine();
                let y2 = base2.mul_bigint(witness.into_bigint()).into_affine();
                let blinding = Fr::rand(&mut rng);
                let protocol1 = Protocol3::<<Bls12_381 as Pairing>::G1Affine>::init(
                    witness, blinding, &base1
                );
        
                let protocol2 = Protocol3::<<Bls12_381 as Pairing>::G1Affine>::init(
                    witness, blinding, &base2,
                );

                let mut chal_contrib_prover = vec![];
                protocol1
                    .challenge_contribution(&base1, &y1, &mut chal_contrib_prover)
                    .unwrap();
                protocol2
                    .challenge_contribution(&base2, &y2, &mut chal_contrib_prover)
                    .unwrap();
                // test_serialization!(
                //     $protocol_name<<Bls12_381 as Pairing>::$group_affine>,
                //     protocol
                ...
                let proof1 = protocol1.gen_proof(&challenge_prover);
                let proof2 = protocol2.gen_proof(&challenge_prover);

                let mut chal_contrib_verifier = vec![];
                proof1
                    .challenge_contribution(&base1, &y1, &mut chal_contrib_verifier)
                    .unwrap();
                proof2
                    .challenge_contribution(&base2, &y2, &mut chal_contrib_verifier)
                    .unwrap();

                let challenge_verifier =
                    compute_random_oracle_challenge::<Fr, Blake2b512>(&chal_contrib_verifier);
                assert!(proof1.verify(&y1, &base1, &challenge_verifier));
                assert!(proof2.verify(&y2, &base2, &challenge_verifier));
                assert_eq!(proof1.response,proof2.response);
                println!("resp 1: {}",proof1.response);
                println!("resp 2: {}",proof2.response);
                assert_eq!(chal_contrib_prover, chal_contrib_verifier);
                assert_eq!(challenge_prover, challenge_verifier);
        ...
        fn check_equality_with_neg_base() {
            // ($protocol_name:ident, $proof_name: ident, $group_affine:ident, $group_projective:ident) => {
                impl_proof_of_knowledge_of_discrete_log!(Protocol3, Proof3);
                let mut rng = StdRng::seed_from_u64(0u64);
                let gbase = <Bls12_381 as Pairing>::G1::rand(&mut rng).into_affine();
                ...
                let base1 = gbase.mul_bigint(z1.into_bigint()).into_affine();
                let base2 = gbase.mul_bigint(z2.into_bigint()).into_affine();
                let base2_neg = -base2.clone();
                let witness = Fr::rand(&mut rng);
                let y1 = base1.mul_bigint(witness.into_bigint()).into_affine();
                let y2 = base2_neg.mul_bigint(witness.into_bigint()).into_affine();
                let blinding = Fr::rand(&mut rng);
                let protocol1 = Protocol3::<<Bls12_381 as Pairing>::G1Affine>::init(
                    witness, blinding, &base1
                );
                ...
                protocol1
                    ...
                    .unwrap();
                protocol2
                    .challenge_contribution(&base2_neg, &y2, &mut chal_contrib_prover)
                    .unwrap();
                // test_serialization!(
                //     $protocol_name<<Bls12_381 as Pairing>::$group_affine>,
                //     protocol
                ...
                proof1
                    ...
                    .unwrap();
                proof2
                    .challenge_contribution(&base2_neg, &y2, &mut chal_contrib_verifier)
                    .unwrap();

                let challenge_verifier =
                    compute_random_oracle_challenge::<Fr, Blake2b512>(&chal_contrib_verifier);
                assert!(proof1.verify(&y1, &base1, &challenge_verifier));
                assert!(proof2.verify(&y2, &base2_neg, &challenge_verifier));
                assert_eq!(proof1.response,proof2.response);
                println!("resp 1: {}",proof1.response);
                println!("resp 2: {}",proof2.response);
                assert_eq!(chal_contrib_prover, chal_contrib_verifier);
